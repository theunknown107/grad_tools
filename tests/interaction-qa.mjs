...
/* ---------------------------------------------------------------------- */
/* The announcement feed this harness supplies for itself                  */
/* ---------------------------------------------------------------------- */

/**
 * Synthetic notices, served to the page by the harness.
 *
 * This used to read whatever the local database happened to contain, which
 * made the check pass or fail on seed state nobody had prepared — and on an
 * empty table it failed for a reason that had nothing to do with the product.
 * Worse, it hid a real defect: the assertion looked for the word "synthetic"
 * while the product labels demo content "Demo data", so the check could not
 * have passed even with a full database.
 *
 * The harness now owns its own feed. No seeding, no database, no fixture to
 * prepare, and the rows exercise the two things the screen actually has to do:
 * label a synthetic notice as synthetic, and narrow the list when a student
 * asks for what applies to them.
 *
 * NOTHING HERE IS REAL. No real college, no real deadline, no real notice.
 */
function announcementFeed() {
  const at = (days) => new Date(Date.UTC(2026, 8, 7 + days, 9, 0, 0)).toISOString();
  const audience = (over) => ({
    schemeId: null,
    branchId: null,
    branchName: null,
  ...
  const notice = (id, over) => ({
    ...
    sourceId: null,
    origin: 'demo_fixture',
    publisher: 'Example Board of Examinations',
    title: `Synthetic notice ${id}`,
    body: 'Generated by tests/interaction-qa.mjs. Not a real announcement.',
    category: 'general',
    canonicalUrl: null,
    publishedAt: at(-2),
    eventStartAt: null,
    ...
    updatedAt: at(-2),
    ...over,
    audience: audience(over?.audience ?? {}),
  });

  return [
    /* Untargeted: applies to everyone, so it survives the relevance filter. */
    notice('demo-1', { category: 'exam_timetable', deadlineAt: at(3) }),
    /* Targeted at a semester the seeded student is NOT in, so the filter bites. */
    notice('demo-2', { category: 'fees', audience: { semester: 8 } }),
    notice('demo-3', { category: 'revaluation', audience: { semester: 7 } }),
    /* An operator entry, to prove the demo label is driven by origin and is
     * not simply printed on every row. */
    notice('entry-1', { origin: 'operator_entry', title: 'Synthetic operator entry' }),
  ];
}
...
const run = async () => {
  ...
  /*
   * The feed is answered by the harness, on every context it opens. The API is
   * covered by its own tests; what this harness asks is what the SCREEN does
   * with a feed, and that question should not depend on a database being up
   * and seeded by hand.
   */
  const feed = announcementFeed();
  for (const target of [desktop]) {
    await target.route('**/api/v1/announcements*', async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({ data: feed, total: feed.length, limit: 20, offset: 0 }),
      });
    });
  }
  ...
  await check('Announcements: the relevance tabs filter the feed', async () => {
    ...
    /*
     * A non-vacuous check. `mine <= all` holds trivially when both are zero,
     * which is what an empty feed used to give it — the assertion passed while
     * proving nothing. The feed is the harness's own now, so the counts are
     * known: four notices, two of them targeted at semesters this student is
     * not in.
     */
    expect(all === 4, `expected the harness feed of 4 notices, saw ${String(all)}`);
  ...
  await check('Announcements: demo notices carry the demo label', async () => {
    /*
     * THIS CHECK USED TO BE UNPASSABLE. It looked for the word "synthetic"
     * while the product labels demo content "Demo data", so it could only ever
     * fail — and an empty announcements table made that look like an
     * environment problem rather than a broken assertion.
     *
     * The label must also be driven by ORIGIN rather than printed on every
     * row, so the operator entry in the feed must NOT carry it. A label that
     * appears on everything distinguishes nothing.
     */
    await page.click('[role="tab"]:has-text("All")');
    await page.waitForTimeout(400);
    const labels = await page.locator('article:has-text("Demo data")').count();
    expect(labels === 3, `expected 3 rows labelled "Demo data", saw ${String(labels)}`);

    const operator = page.locator('article:has-text("Synthetic operator entry")');
    expect(
      (await operator.count()) === 1 && !/Demo data/.test(await operator.first().innerText()),
      'the operator entry was labelled as demo content',
    );
  });
