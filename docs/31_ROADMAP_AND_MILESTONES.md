...
**Re-scoped by the attendance ledger, and the scope is now written down.** M9 was built when attendance was two integers per subject, and its account carried them like any other record. With the per-class ledger those integers became a DERIVED CACHE, and a device on `schemaVersion >= 1` publishes no attendance row at all — because two devices each deriving their own figure from their own ledger and writing one shared row cannot settle. What an account syncs is therefore an allowlist, stated in `07` §7.16 and `11` §11.15 rather than left to be inferred: the academic record syncs; the per-class ledger, the date-specific timetable overrides and the observations made of a synced total are **device-local in this phase**. The product says so on Settings → Data & privacy and on the attendance Date view, and `apps/web/test/auth-boundaries.test.ts` asserts the allowlist as data so the boundary cannot drift quietly.

**Verified again after the attendance work.** The account lifecycle was re-proven against real row-level security on the same migration Supabase runs: one `auth_user_id` gets exactly one profile however often it signs in, the profile id is stable across sessions, an email change re-keys nothing, two accounts cannot see each other, deletion removes one account and leaves the other, and a deleted account that signs in again starts empty (`services/api/test/account-lifecycle.test.ts`). The browser build was scanned for credential shapes rather than names (`apps/web/test/bundle-secrets.test.ts`).

**Still NOT VERIFIED, and not claimable:** Google and Apple. Neither is configured on any project this repository can reach, and a checkout with no `.env` has no project at all — so no provider round trip was performed in that state, and none is reported as if it had been. What changed is only the honesty of the failure: a provider a project has not enabled now tells the student the method is unavailable rather than "something went wrong" (`11` §11.16).
